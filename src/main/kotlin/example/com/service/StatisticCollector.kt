...
class StatisticCollector(private val logger: Logger) {
    ...
    fun logStats() {
        ...
        logger.info(
            "Process duration: ${processDuration.get().nanoseconds}  parallelization ${
                (processDuration.get().toFloat() / total)
            }"
        )
        logger.info(
            "Request duration: ${requestDuration.get().nanoseconds}  parallelization ${
                (requestDuration.get().toFloat() / total)
            }"
        )
